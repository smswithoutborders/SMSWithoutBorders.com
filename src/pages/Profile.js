...
const QRContainer = tw(
  QRCode
)`block mx-auto border shadow-lg rounded-xl p-4 bg-white`;
    ...
    if (!Object.keys(userProfile).length > 0) {
      ...
      getProfileInfo(id, token)
        ...
        .catch((error) => {
          if (error.response) {
            toast.error(
              "Request Error \n Sorry we could not locate your profile. Please check your network connection and try again"
            );
          } else if (error.request) {
            toast.error(
              "Network Error \n We could not fetch your profile. Please check your network and reload this page"
            );
          } else {
            toast.error(
    ...
    return (
      <PageAnimationWrapper>
        <SectionContainer>
          <ImageContainer>
            {syncState.connected && (
              <QRContainer value={syncState.qrLink} size={300} />
            )}
          ...
          <DetailsContainer>
            ...
            <Meta>
              This lets you use the SW/OB secure gateway for all messages
            </Meta>
            <br />
    ...
    <PageAnimationWrapper>
      <SectionContainer>
        ...
        <DetailsContainer>
          <Heading>
            {" "}
            Welcome, <ProfileName>{userProfile?.name}</ProfileName>{" "}
          </Heading>
